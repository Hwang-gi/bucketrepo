...
<!--
227번째 줄: 데이터베이스 연결 정보 (dbURL, dbUser, dbPassword) 설정
227번째 줄: MariaDB 드라이버 로드
227번째 줄: 데이터베이스 연결
241번째 줄: SQL 쿼리 실행 (services2 테이블에서 서비스 이름, 설명, 가격, 이미지 URL을 가져옴)
280번째 줄: 50% 할인된 가격 적용
130번째 줄: 예약하기 버튼 클릭 시 모달 창에서 날짜 선택
195번째 줄: 로고 클릭 시 main.jsp로 이동
202번째 줄: HOTEL 클릭 시 order.jps로 이동
202번째 줄: CONTACT 클릭 시 reservation.jsp로 이동
210번째 줄: 우측 로그인 버튼 클릭 시 login.jsp로 이동
-->
    ...
    <div class="containerBox">
        <div class="container">
            ...
            <div class="services">
                <% 
                    // DB 연결 설정
                    String dbURL = "jdbc:mariadb://10.240.2.201/yas";
                    String dbUser = "root"; 
                    ...
                    try {
                        ...
                        while (rs.next()) {
                            ...
                            double discountPrice = price * 0.5; // 50% 할인 적용
                %>
                <!-- HTML로 서비스 출력 -->
                <div class="service-item">
                    ...
                    <p><%= description %></p>
                    <p class="price">
                        <span class="original-price">₩<%= String.format("%,.0f", price) %></span>  <!-- 취소선 가격 -->
                        <span class="discount-price">₩<%= String.format("%,.0f", discountPrice) %></span>  <!-- 할인 가격 -->
                    </p>
